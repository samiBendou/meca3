 ...
 * ```js
 * let vertices = [Vector3.ex, Vector3.ey, Vector3.exn, Vector3.eyn];
 * let dt = 0.1;
 * ```
 *
 ...
 * ```
 *
 * **Note** If `vertices` is of size N then `dt` is of size N-1.
 *
 * Then choose your origin and construct the curve :
 *
 * ```js
 * const origin = Vector3.zeros
 * let curve = new Curve(vertices, origin, dt); // origin is (0, 0, 0)
 * ```
 *
 * **Note** It's preferable that the origin does not reference an object contained in the `vertices` array and that references
 * in `vertices` points to different instances.
 *
 * ### vertices
 * The `vertices` array stores position of curve samples relative to `this.origin`, when affecting a new origin,
 * the new relative vertices are recomputed. It's the same process as what was done for [[Point3]] class.
 *
 * ### LIFO Structure
 ...
 */
export default interface Curve<T extends Vector> {
  ...
  transform<U extends Matrix<T>>(m: U): this;

  affine<U extends Matrix<T>>(m: U, v: T): this;
