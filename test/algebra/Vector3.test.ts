...
import { Vector3 } from "../../src/algebra";
...
describe("Vector3 Tests", () => {
  ...
  describe("Algebra", () => {
    it("adds", () => assert3.equal(magic.clone().add(ones), [2, 3, 4]));
    it("subtracts", () => assert3.equal(magic.clone().sub(ones), [0, 1, 2]));
    it("negates", () => assert3.equal(magic.clone().neg(), [-1, -2, -3]));
    it("multiplies with scalar", () =>
      assert3.equal(magic.clone().mul(2), [2, 4, 6]));
    it("multiplies with vector", () =>
      assert3.equal(magic.clone().prod(ey), [0, 2, 0]));
    it("inverses with vector", () =>
      assert3.equal(magic.clone().inv(), [1, 1 / 2, 1 / 3]));
    it("divides", () => assert3.equal(magic.clone().div(0.5), [2, 4, 6]));
    it("interpolates", () =>
      assert3.equal(ex.clone().lerp(ex.clone().mul(2), 0.5), [1.5, 0, 0]));
    it("can assign during operation", () =>
      assert3.equal(magic.clone().add(ones), [2, 3, 4]));
  });
  ...
  describe("Geometry", () => {
    ...
    describe("Cross Product", () => {
      it("gets ex x ey", () => assert3.equal(ex.clone().cross(ey), ez));
      it("gets ey x ez", () => assert3.equal(ey.clone().cross(ez), ex));
      it("gets ez x ex", () => assert3.equal(ez.clone().cross(ex), ey));
      it("gets ex x ex", () => assert3.equal(ex.clone().cross(ex), zeros));
    });
  ...
  describe("Serialization", () => {
    ...
    it("decodes to array", () => assert3.equal(Vector3.array(aD), ex));
