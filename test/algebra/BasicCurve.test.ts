...
import { BasicCurve } from "../../src/curves";
...
describe("Curve Tests", function () {
  ...
  describe("Initialization", () => {
    it("should has the right size", () => {
      assert.equal(square.vertices.length, 5);
    });
    it("should set the elements", () => {
      assert3.equal(square.vertices, [ex, ey, exn, eyn, ex]);
    });
  });
  ...
  describe("Push/Pop", () => {
    describe("Push without dt", () => {
      it("should push position", () =>
        assert3.equal(zeros.push(ex).vertices[1], ex));
    });
    ...
    describe("Pop", () => {
      ...
      it("should pop position", () => {
        square.pop();
        assert.equal(square.vertices.length, 4);
  ...
  describe("Geometry", () => {
    it("should translate the curve", () =>
      assert3.equal(linear0.translate(ex.clone()).vertices, [
        Vector3.zeros,
        Vector3.ex,
        Vector3.ex.mul(2),
      ]));
    it("should rotate the curve", () =>
      assert3.approximately(
        square.transform(Matrix3.rotZ(Math.PI / 2)).vertices,
  ...
  describe("Cinematic", () => {
    describe("Position", () => {
      ...
      it("should get middle position", () =>
        assert3.equal(linear1.positionAt(0.5), linear1.vertices[1]));
      it("should get x = 0.25 position", () =>
        assert3.equal(linear1.positionAt(0.25), ex.clone().mul(0.5)));
      it("should get x = 0.75 position", () =>
        assert3.equal(linear1.positionAt(0.75), ex.clone().mul(-0.5)));
    });
    ...
    describe("Generators", () => {
      it("gets zeros", () =>
        assert3.equal(BasicCurve.constant(2, ex).vertices, [ex, ex]));
      it("gets graph", () =>
        assert3.equal(
          BasicCurve.graph(
